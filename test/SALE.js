...
contract('Sale', function([_, wallet, wallet2, wallet3]) {
  ...
  beforeEach(async function() {
    ...
    await this.sale.addAddressesToWhitelist([_, wallet]);
  });
  ...
  describe('WhiteList', function() {
    it('User deleted from whiteList can not be able buy Tokens', async function() {
      await this.sale.removeAddressFromWhitelist(wallet);
      await this.sale.sendTransaction({
        value: ether(1),
        from: wallet
    ...
    it('User can not call removeAddressFromWhitelist', async function() {
      await this.sale.removeAddressFromWhitelist(_, {
        from: wallet
      }).should.be.rejectedWith(EVMRevert);
    });

    it('User can not call addAddressToWhitelist', async function() {
      await this.sale.addAddressToWhitelist(wallet2, {
        from: wallet
      }).should.be.rejectedWith(EVMRevert);
    });

    it('User can not call addAddressesToWhitelist', async function() {
      await this.sale.addAddressesToWhitelist([wallet2, wallet3], {
        from: wallet
      }).should.be.rejectedWith(EVMRevert);
    });

    it('User can not call removeAddressFromWhitelist', async function() {
      await this.sale.removeAddressesFromWhitelist([wallet2, wallet3], {
        from: wallet
      }).should.be.rejectedWith(EVMRevert);
    });

    it('Owner can call removeAddressFromWhitelist', async function() {
      await this.sale.removeAddressFromWhitelist(_, {
        from: _
      }).should.be.fulfilled;
    });

    it('Owner can call addAddressToWhitelist', async function() {
      await this.sale.addAddressToWhitelist(wallet2, {
        from: _
      }).should.be.fulfilled;
    });
    ...
    it('Owner can call addAddressesToWhitelist', async function() {
      await this.sale.addAddressesToWhitelist([wallet2, wallet3], {
        from: _
      }).should.be.fulfilled;
    });
